        ...
        <div id="capture-area-overview" className="space-y-6 bg-white p-4 rounded-xl">
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
                    <Users className="text-blue-600" size={20} /> Ï∞∏ÏÑù??Î™ÖÎã® ({players.length}Î™?
                </h3>
                {players.length === 0 ? (
                    <p className="text-slate-400 text-sm">Ï∞∏ÏÑù?êÍ? ?ÑÏßÅ ?ÜÏäµ?àÎã§. Í¥ÄÎ¶¨ÏûêÍ∞Ä Î™ÖÎã®???ÖÎç∞?¥Ìä∏???åÍπåÏßÄ Í∏∞Îã§?§Ï£º?∏Ïöî.</p>
                ) : (
            ...
            <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100">
                <h3 className="font-bold text-blue-900 mb-2">?ÅÌÉú: {status === 'recruiting' ? 'Î™®Ïßë Ï§? : status === 'closed' ? 'ÎßàÍ∞ê?? : 'Ï¢ÖÎ£å??}</h3>
                <p className="text-blue-700 text-sm leading-relaxed">
                    {status === 'recruiting'
                        ? '?ÑÏû¨ Ï∞∏ÏÑù?êÎ? ?åÏïÖ Ï§ëÏûÖ?àÎã§. ?¨ÌëúÍ∞Ä ÎßàÍ∞ê?òÎ©¥ ?Ä Íµ¨ÏÑ±???úÏûë?©Îãà??'
                        : 'Ï∞∏ÏÑù ?ïÏù∏???ÑÎ£å?òÏóà?µÎãà?? ?Ä Íµ¨ÏÑ±???ïÏù∏?¥Ï£º?∏Ïöî.'}
                </p>
            </div>
        </div>
    )
...
function TabTeams({ teams, players, onAssign, isAdmin, preferences }: { teams: any[], players: any[], onAssign: (pid: number, tid: number | null) => void, isAdmin: boolean, preferences?: any[] }) {
    ...
    const getTeamCharacter = (stats: { atk: number, mid: number, def: number, phys: number }) => {
        ...
        if (atk === 0 && mid === 0 && def === 0) return { type: 'Î∂ÑÏÑù Î∂àÍ?', emoji: '??, strategy: '?†ÏàòÎ•?Î∞∞Ï†ï?¥Ï£º?∏Ïöî', color: 'text-slate-400' }
        ...
        if (atkRatio > 1.15 && defRatio < 0.9) {
            return {
                type: 'Í≥µÍ≤©??,
                emoji: '?îÔ∏è',
                strategy: '?ÅÍ∑π?ÅÏù∏ ?ïÎ∞ïÍ≥?Îπ†Î•∏ ??äµ???∏Î¶¨?∏Ïöî',
                color: 'text-red-600'
            }
            ...
            return {
                type: '?òÎπÑ??,
                emoji: '?õ°Ô∏?,
                strategy: 'Í≤¨Í≥†???òÎπÑ ??Ïπ¥Ïö¥???¥ÌÉù???∏Î¶¨?∏Ïöî',
                color: 'text-blue-600'
            }
            ...
            return {
                type: '?êÏú†??,
                emoji: '?éØ',
                strategy: 'Î≥??êÏú†Î•??íÏù¥Í≥??®Ïä§Î°?Í≥µÍ∞Ñ??ÎßåÎìú?∏Ïöî',
        ...
        return {
            type: 'Î∞∏Îü∞?§Ìòï',
            emoji: '?ñÔ∏è',
            strategy: '?ÅÌô©??ÎßûÍ≤å ?†Ïó∞?òÍ≤å ?Ä?ëÌïò?∏Ïöî',
        ...
        <div id="capture-area-teams" className="space-y-6">
            ...
            <div className="bg-slate-100 p-4 rounded-xl border border-dashed border-slate-300">
                <h3 className="font-bold text-slate-500 mb-3 flex items-center gap-2 text-sm uppercase tracking-wide">
                    <Users size={14} /> ÎØ∏Î∞∞???†Ïàò (?ÄÍ∏?Î™ÖÎã®)
                </h3>
                {unassigned.length === 0 ? (
                    <p className="text-xs text-slate-400">?ÄÍ∏?Ï§ëÏù∏ ?†ÏàòÍ∞Ä ?ÜÏäµ?àÎã§.</p>
                ) : (
                    <div className="flex flex-wrap gap-2">
                        {unassigned.map(p => (
                            <div key={p.id} className="bg-white px-3 py-2 rounded-lg shadow-sm border border-slate-200 flex items-center gap-2">
                                ...
                                {isAdmin && (
                                    <select
                                        ...
                                        value=""
                                    >
                                        <option value="" disabled>?Ä ?†ÌÉù...</option>
                                        {teams.map(t => (
                        ...
                        <div key={team.id} className={cn("p-5 rounded-2xl border shadow-sm transition-all hover:shadow-md flex flex-col h-full", style.bg, style.border)}>
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs text-slate-400">??{team.players?.length || 0}Î™?/span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {team.players?.length > 0 && (
                                <div className={cn("mb-3 p-2 rounded-lg border", aiInfo ? "bg-gradient-to-r from-purple-50 to-indigo-50 border-purple-100" : "bg-white/60 border-white/80")}>
                                    <div className="text-[10px] text-slate-500 font-medium">{aiInfo ? '?§ñ AI ?ÑÎûµ' : '?í° ?ÑÎûµ ??}</div>
                                    <div className="text-xs text-slate-700 font-medium">{displayInfo.strategy}</div>
                                    {aiInfo?.keyPlayer && (
                                        <div className="mt-1 text-[10px] text-purple-600">
                                            ‚≠??µÏã¨: <span className="font-bold">{aiInfo.keyPlayer}</span> - {aiInfo.keyPlayerReason}
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            <div className="mb-4 space-y-2 bg-white/40 p-3 rounded-xl">
                                ...
                                <div className="flex items-center gap-2 text-[10px] font-bold text-slate-600">
                                    <span className="w-8">ÎØ∏Îìú</span>
                                    <div className="flex-1 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                            ...
                            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1 flex-1">
                                {team.players && team.players.length > 0 ? (
                                        ...
                                        <div key={m.id} className="flex items-center justify-between p-2 rounded-lg bg-white/60 hover:bg-white/80 transition-colors group">
                                            ...
                                            {isAdmin && (
                                                <select
                                                    ...
                                                    value={team.id}
                                                >
                                                    <option value={team.id} disabled>?¥Îèô</option>
                                                    <option value="unassign">ÎØ∏Î∞∞??/option>
                                    ...
                                    <div className="text-sm opacity-50 text-center py-4">?Ä?êÏù¥ ?ÜÏäµ?àÎã§.</div>
                                )}
                            </div>
                        </div>
                    )
...
function TabScoreboard({ matches, teams, onUpdateMatch, isAdmin, canRecord, onRecordEvent, onAddMatch, onDeleteMatch, onClearMatches, onRegenMatches, onAutoFillMatches, sessionId }: {
    ...
    const handleSaveGoal = () => {
        if (!goalModal || !selectedScorer) return alert('?ùÏ†ê?êÎ? ?†ÌÉù?¥Ï£º?∏Ïöî')
    ...
    return (
        <div id="capture-area-scoreboard" className="space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                <table className="w-full text-sm text-center">
                    <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-100 text-xs uppercase tracking-wider">
                        <tr>
                            <th className="px-4 py-3 w-12 text-left whitespace-nowrap">?úÏúÑ</th>
                            <th className="px-4 py-3 text-left whitespace-nowrap">?Ä</th>
                            <th className="px-2 py-3 whitespace-nowrap">Í≤ΩÍ∏∞</th>
                            <th className="px-2 py-3 text-slate-400 whitespace-nowrap">??/th>
                            <th className="px-2 py-3 text-slate-400 whitespace-nowrap">Î¨?/th>
                            <th className="px-2 py-3 text-slate-400 whitespace-nowrap">??/th>
                            <th className="px-2 py-3 whitespace-nowrap">?ùÏã§</th>
                            <th className="px-4 py-3 font-bold text-blue-600 whitespace-nowrap">?πÏ†ê</th>
                        </tr>
                    </thead>
            ...
            <div className="space-y-4">
                ...
                {matches.length === 0 && <p className="text-slate-400 text-center py-10">?ºÏ†ï???ÜÏäµ?àÎã§.</p>}
            ...
            {goalModal && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
                        <h3 className="font-bold text-lg text-center">Í≥?Í∏∞Î°ù</h3>
                        <div>
                            <label className="text-xs font-bold text-slate-500">?ùÏ†ê??(Í≥?</label>
                            <select
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-500">?ÑÏ? (?¥Ïãú?§Ìä∏)</label>
                            <select
                                value={selectedAssister}
                                onChange={e => setSelectedAssister(e.target.value)}
                                className="w-full mt-1 p-3 bg-slate-50 rounded-xl font-bold border border-slate-200"
                            >
                                <option value="">?ÜÏùå (Í∞úÏù∏ ?åÌåå)</option>
                        ...
                        <div className="flex gap-2 pt-2">
                            <button onClick={() => setGoalModal(null)} className="flex-1 py-3 bg-slate-100 rounded-xl font-bold text-slate-600">Ï∑®ÏÜå</button>
                            <button onClick={handleSaveGoal} className="flex-1 py-3 bg-blue-600 rounded-xl font-bold text-white shadow-lg shadow-blue-500/30">Í∏∞Î°ù ?Ä??/button>
                        </div>
                    </div>
                </div>
            )}
...
function getSeasonTitle(dateStr: string) {
    ...
    if (msDiff < 0) return `${year} ?ÑÎ¶¨?úÏ¶å`
...
export default function SessionDetail() {
    ...
    useEffect(() => {
        fetch(`${API_URL}/sessions/${id}`)
            ...
            .catch(() => setError('?∏ÏÖò??Ï∞æÏùÑ ???ÜÍ±∞???∞Í≤∞ ?§Ìå®'))
    }, [id])
    ...
    const handleSaveAttendees = async () => {
        ...
        const confirmMsg = parseResult.unknown.length > 0
            ? `Í∏∞Ï°¥ ?åÏõê ${parseResult.matched.length}Î™ÖÍ≥º ?†Í∑ú(ÎØ∏Îì±Î°? ${parseResult.unknown.length}Î™ÖÏùÑ ?¨Ìï®?òÏó¨ Ï¥?${totalCount}Î™ÖÏùÑ ?±Î°ù?òÏãúÍ≤†Ïäµ?àÍπå?\n(?†Í∑ú ?åÏõê?Ä ?êÎèô ?ùÏÑ±?©Îãà??`
            : `Ï¥?${totalCount}Î™ÖÏùò Ï∞∏ÏÑù?êÎ? ?±Î°ù?òÏãúÍ≤†Ïäµ?àÍπå?`
        ...
        if (parseResult.unknown.length > 0) {
            try {
                ...
                alert('?†Í∑ú ?åÏõê ?ùÏÑ± Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.')
                return
            }
        }
    ...
    const handleStatusChange = async (newStatus: string) => {
        if (newStatus === 'closed' && !confirm('ÎßàÍ∞ê Ï≤òÎ¶¨ ?òÏãúÍ≤†Ïäµ?àÍπå? ???¥ÏÉÅ Ï∞∏ÏÑù?êÎ? ?òÏ†ï?????ÜÍ≤å ?©Îãà??')) return
        if (newStatus === 'recruiting' && !confirm('ÎßàÍ∞ê??Ï∑®ÏÜå?òÍ≥† ?§Ïãú Î™®Ïßë Ï§ëÏúºÎ°?Î≥ÄÍ≤ΩÌïò?úÍ≤†?µÎãàÍπ?')) return
    ...
    const handleGenerateTeams = async () => {
        if (!confirm('Í∏∞Ï°¥ ?Ä Íµ¨ÏÑ±??Î™®Îëê Ï¥àÍ∏∞?îÎêòÍ≥??àÎ°ú ?ùÏÑ±?©Îãà?? Í≥ÑÏÜç?òÏãúÍ≤†Ïäµ?àÍπå? (??ñ¥?∞Í∏∞)')) return
        try {
            const token = localStorage.getItem('auth_token') || localStorage.getItem('token')
            ...
            if (res.ok && data.success) {
                ...
                const msg = `???Ä ?ùÏÑ± ?ÑÎ£å!\n\nÎ∞∏Îü∞???êÏàò: ${score}??n\n` +
                    `?ìã ?ùÏÑ± ?îÏïΩ:\n` +
                    `- ${data.teams?.length || 0}Í∞??Ä Íµ¨ÏÑ±\n` +
                    `- ${data.match_count || 0}Í≤ΩÍ∏∞ ?ùÏÑ±\n` +
                    (score >= 80 ? '‚≠??ÑÏ£º Í∑†Ìòï?°Ìûå ?Ä Íµ¨ÏÑ±?ÖÎãà??' :
                        score >= 60 ? '?ëç ?ÅÏ†à??Î∞∏Îü∞?§ÏûÖ?àÎã§.' :
                            '?†Ô∏è ?†Ïàò ?•Î†•Ïπ??∏Ï∞®Í∞Ä ?àÏäµ?àÎã§.')
                alert(msg)
                window.location.reload()
            } else {
                alert('?Ä ?ùÏÑ± ?§Ìå®: ' + (data.error || '?????ÜÎäî ?§Î•ò'))
            }
        } catch (error) {
            console.error(error)
            alert('?Ä ?ùÏÑ± Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.')
        }
    }
    ...
    const handleClearMatches = async () => {
        if (!confirm('Î™®Îì† Í≤ΩÍ∏∞Î•???†ú?òÏãúÍ≤†Ïäµ?àÍπå?')) return
        await fetch(`${API_URL}/sessions/${id}/matches`, { method: 'DELETE' })
        refreshSession()
    }

    const handleRegenMatches = async () => {
        if (!confirm('Í∏∞Ï°¥ Í≤ΩÍ∏∞Î•???†ú?òÍ≥† 9Í≤ΩÍ∏∞Î•??àÎ°ú ?ùÏÑ±?©Îãà??')) return
    ...
    const handleAddMatch = async () => {
        if (!session.teams || session.teams.length < 2) return alert('?Ä??ÏµúÏÜå 2Í∞??ÑÏöî?©Îãà??')
    ...
    const handleDeleteMatch = async (mid: number) => {
        if (!confirm('Í≤ΩÍ∏∞Î•???†ú?òÏãúÍ≤†Ïäµ?àÍπå?')) return
    ...
    if (error) return (
        <div className="p-20 text-center">
            <h2 className="text-xl font-bold text-slate-700 mb-2">?§Î•ò Î∞úÏÉù</h2>
            <p className="text-slate-500">{error}</p>
            <button onClick={() => window.location.reload()} className="mt-4 px-4 py-2 bg-slate-200 rounded text-slate-700 font-bold">?àÎ°úÍ≥†Ïπ®</button>
        </div>
    )

    if (!session) return <div className="p-20 text-center text-slate-500 animate-pulse">?∞Ïù¥??Î°úÎî© Ï§?..</div>

    const tabs = [
        { id: 'overview' as const, label: 'Í∞úÏöî/Ï∞∏ÏÑù', icon: ClipboardList },
        { id: 'teams' as const, label: '?Ä Íµ¨ÏÑ± (?òÎèô)', icon: Shield },
        { id: 'scoreboard' as const, label: '?êÏàò??, icon: Activity },
    ]
            ...
            <div className="mb-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4 text-slate-500 text-sm font-medium">
                        <span className="flex items-center gap-1"><MapPin size={16} /> Í≤ΩÎ∂Å?Ä AÍµ¨Ïû•</span>
                    </div>
                </div>
            </div>
            ...
            {isAdmin && (
                <div className="bg-slate-900 rounded-2xl p-6 mb-8 text-white shadow-xl">
                    <h2 className="font-bold text-lg mb-4 flex items-center gap-2">
                        <Lock size={18} className="text-yellow-400" /> Í¥ÄÎ¶¨Ïûê Ïª®Ìä∏Î°??®ÎÑê
                    </h2>
                    ...
                    {session.status === 'recruiting' && (
                        <div className="space-y-4 mb-8 border-b border-slate-700 pb-6">
                            <label className="text-sm font-bold text-slate-300 flex items-center gap-2">
                                <Wand2 size={14} /> Ïπ¥Ïπ¥?§ÌÜ° ?¨Ìëú Î∂ôÏó¨?£Í∏∞ (Ï∞∏ÏÑù??Í∞±Ïã†)
                            </label>
                            <textarea
                                ...
                                placeholder="Ïπ¥ÌÜ° ?¥Ïö©??Î∂ôÏó¨?£Ïúº?∏Ïöî..."
                                value={parseText}
                            ...
                            <div className="flex gap-2">
                                ...
                                {parseResult && (
                                    ...
                                    >
                                        2. Í≤∞Í≥º ?Ä??(Í∏∞Ï°¥ {parseResult.matched.length}Î™?+ ?†Í∑ú {parseResult.unknown.length}Î™?
                                    </button>
                                )}
                            </div>
                        ...
                        {session.status === 'recruiting' ? (
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    title="ÎßàÍ∞ê Ï∑®ÏÜå (?§Ïãú Î™®Ïßë Ï§ëÏúºÎ°?Î≥ÄÍ≤?"
                                >
                                    <RefreshCw size={18} /> ÎßàÍ∞ê Ï∑®ÏÜå
                                </button>
                            </div>
                        )}
